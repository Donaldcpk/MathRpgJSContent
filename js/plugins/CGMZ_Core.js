 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this latest version should make a big difference for scrolling CGMZ
 * windows for gamepad (controller) players. They should now always be able to
 * scroll even if not active provided the player uses the right stick. Keyboard
 * and mouse players have already been able to do this as touch scrolling has
 * always been active in these windows.
 * 
 * Previously, they would need to first activate the scrolling window which
 * was typically done by selecting an item in a list and then using the left
 * stick. This was confusing ux because the selection window would then be
 * deactivated so the scrolling window could scroll via input. This
 * deactivation led to some confusion as it could look like nothing happened,
 * and now the select window was not responding. With this change, the
 * scrolling window no longer needs to be selected at all and gamepad users
 * will still be able to manually scroll it.
 *
 * Due to this update, I will most likely be going over my plugins with
 * scrolling windows in the future and allowing you the option to prevent
 * the list window that activates them from needing to allow for selection.
 *
 * This update added a utility function to parse rumble json. This should make
 * [CGMZ] Rumble integration much easier in the future.
 *
 * This version also removes some code that has been deprecated for a long
 * time now. All [CGMZ] plugins have since been updated to no longer use it, so
...
var CGMZ = {};
CGMZ.Versions = {};
CGMZ.Versions["CGMZ Core"] = "1.22.0";
CGMZ.Core = {};
